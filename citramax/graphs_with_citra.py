...
l_int = [0.97, 0.94, 0.9]  # lower uncertainty constant
u_int = [1.03, 1.06, 1.1]  # upper uncertainty constant
...
for count, (o, p) in enumerate(zip(l_int, u_int)):
	if o == 0.7 and p == 1.3:
		stoich_reactions = stoich_reactions_2
		x = x_2
		h = h_2
	...
	ax2.set_ylabel('Citramalate flux (mmol $g_{DW}^{-1}$ $h^{-1}$)' , color="#236BA1", fontsize=24)
	...
	ax.set_ylabel('Citramalate production flux (mmol $g_{DW}^{-1}$ $h^{-1}$)', fontsize = 15)
...
### CONVERSION TO CITRAMALATE

fig, ax = plt.subplots(figsize=(10,6))

x = ['Experimental', 'Original', 'Uncertainty d=0.1', 'Uncertainty d=0.06', 'Uncertainty d=0.03']
y = [0.48, 0.98, 0.753, 0.74, 0.73]

ax.bar(x, y)
ax.grid(True, color='gray', linestyle='--', linewidth=0.5, alpha=0.7)
ax.set_ylabel('Conversion efficiency (g citramalate/g glucose)', fontsize=14)
ax.set_xlabel('Condition', fontsize=14)
ax.tick_params(axis="y", labelsize = 12, labelrotation = 30)
