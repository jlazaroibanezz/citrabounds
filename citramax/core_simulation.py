...
l_int = 0.9  # lower uncertainty constant
u_int = 1.1  # upper uncertainty constant
...
def core_sim(stoich_reactions, g, tkt1_f, tkt2_f, tala_f, l_int =[], u_int=[]):
    ...
    const_model = cobra.io.read_sbml_model("iML1515CITRA.xml")
    const_model.solver = 'cplex'
    gluc_fluxmin = const_model.reactions.get_by_id("EX_glc__D_e").lower_bound = -C*g
    gluc_fluxmax = const_model.reactions.get_by_id("EX_glc__D_e").upper_bound = -C*g
    growth_flux = 0.0015767943381718983
    const_model.objective = 'EX_Citramalate'
    growth_fluxmin = const_model.reactions.get_by_id("BIOMASS_Ec_iML1515_WT_75p37M").lower_bound = growth_flux*l_int
    growth_fluxmax = const_model.reactions.get_by_id("BIOMASS_Ec_iML1515_WT_75p37M").upper_bound = growth_flux*u_int
    ...
    for z, l in zip(stoich_reactions[15:], flux_kin[19:]):
